...
### 🔑 Різниця між підписанням та хешуванням

Багато плутають цифрові підписи з хешуванням, але це різні криптографічні операції:
...
**Характеристики хешування:**
- ✅ Перевіряє **цілісність** (чи не змінилися дані)
- ❌ НЕ перевіряє **автентичність** (хто створив дані)
- ✅ Будь-хто може обчислити hash
- ❌ Якщо зловмисник змінить повідомлення, він може створити новий hash

**Проблема:** Якщо Alice надсилає `message + hash`, зловмисник може:
1. Змінити повідомлення: "I authorize this transaction" → "I authorize $1,000,000"
2. Перерахувати hash для нового повідомлення
3. Bob отримає підроблене повідомлення з валідним hash!

#### Цифровий підпис (Digital Signature)

```python
# Alice підписує приватним ключем
signature = sign_message(message, 'alice_private_key.pem')

# Bob перевіряє публічним ключем Alice
is_valid = verify_signature(message, signature, 'alice_public_key.pem')
```

**Характеристики підпису:**
- ✅ Перевіряє **цілісність** (чи не змінилися дані)
- ✅ Перевіряє **автентичність** (хто підписав)
- ✅ Перевіряє **неможливість відмови** (Alice не може заперечити підпис)
- ✅ Тільки власник приватного ключа може створити підпис
- ✅ Будь-хто з публічним ключем може перевірити підпис

**Захист:** Якщо зловмисник змінить повідомлення:
1. Старий підпис стане невалідним
2. Зловмисник не може створити новий підпис (немає приватного ключа Alice)
3. Bob виявить підробку при перевірці підпису

#### Порівняльна таблиця

| Характеристика | Хешування | Цифровий підпис |
|----------------|-----------|-----------------|
| **Цілісність** | ✅ Так | ✅ Так |
| **Автентичність** | ❌ Ні | ✅ Так |
| **Потрібен ключ** | ❌ Ні | ✅ Так (приватний для підпису) |
| **Хто може створити** | Будь-хто | Тільки власник приватного ключа |
| **Захист від підробки** | ❌ Слабкий | ✅ Сильний |
| **Використання** | Checksums, перевірка файлів | Автентифікація документів, транзакцій |

#### Як працює цифровий підпис під капотом?

Цифровий підпис насправді **використовує хешування**, але додає криптографічний захист:

```python
# Спрощена схема того, що відбувається всередині sign_message():

# 1. Обчислити hash повідомлення
message_hash = hashlib.sha256(message.encode()).digest()

# 2. Зашифрувати hash ПРИВАТНИМ ключем (це і є підпис!)
signature = private_key.sign(message_hash, ...)

# Це протилежно звичайному шифруванню:
# - Звичайне шифрування: публічний ключ шифрує → приватний розшифровує
# - Підпис: приватний ключ "шифрує" hash → публічний "розшифровує" (верифікує)
```

**Перевірка підпису:**
```python
# Спрощена схема того, що відбувається всередині verify_signature():

# 1. Обчислити hash отриманого повідомлення
received_hash = hashlib.sha256(message.encode()).digest()

# 2. "Розшифрувати" підпис публічним ключем (отримати оригінальний hash)
signed_hash = public_key.verify(signature, ...)

# 3. Порівняти hash'і
if received_hash == signed_hash:
    print("✅ Підпис валідний - повідомлення не змінене і від Alice")
else:
    print("❌ Підпис невалідний - повідомлення підроблене!")
```

#### Практичний приклад

**Сценарій 1: Тільки hash (небезпечно)**
```python
# Alice надсилає
message = "Transfer $100"
hash_value = hashlib.sha256(message.encode()).hexdigest()
# Alice відправляє: (message, hash_value)

# Зловмисник перехоплює і підробляє:
fake_message = "Transfer $10000"
fake_hash = hashlib.sha256(fake_message.encode()).hexdigest()
# Надсилає Bob: (fake_message, fake_hash)

# Bob перевіряє
received_hash = hashlib.sha256(fake_message.encode()).hexdigest()
if received_hash == fake_hash:
    print("✅ Hash matches!")  # Але це підроблене повідомлення!
```

**Сценарій 2: Цифровий підпис (безпечно)**
```python
# Alice підписує
message = "Transfer $100"
signature = sign_message(message, 'alice_private_key.pem')
# Alice відправляє: (message, signature)

# Зловмисник намагається підробити:
fake_message = "Transfer $10000"
# Він НЕ може створити валідний підпис без приватного ключа Alice!

# Bob перевіряє
is_valid = verify_signature(fake_message, signature, 'alice_public_key.pem')
print(f"Valid: {is_valid}")  # ❌ False - підробка виявлена!
```

#### Коли використовувати що?

**Використовуйте хешування:**
- Перевірка цілісності файлів після завантаження
- Checksums для виявлення корупції даних
- Зберігання паролів (з salt!)
- Blockchain (proof of work)

**Використовуйте цифрові підписи:**
- Аутентифікація документів
- Верифікація оновлень софту
- Підтвердження транзакцій
- Email підписи (PGP/GPG)
- SSL/TLS сертифікати
- Контракти та юридичні документи

### 📝 Поглиблене вивчення криптографії

**Розуміння алгоритмів:**
- [ALGORITHMS_EXPLAINED.md](ALGORITHMS_EXPLAINED.md) - порівняння RSA vs ED25519 vs X25519, коли що використовувати
- [SSH_EXPLAINED.md](SSH_EXPLAINED.md) - як працює SSH з ED25519, key exchange та шифрування з'єднання

**Детальне вивчення RSA:**
- [RSA_EDUCATION_README.md](RSA_EDUCATION_README.md) - повний посібник з навчальними матеріалами про математику RSA
